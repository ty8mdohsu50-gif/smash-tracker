...
export default function MatchLogModal({ open, onClose, title, matches, T, t, lang, isPC }) {
  ...
  const rows = useMemo(() => {
    ...
    if (memoFilter === "with") m = m.filter((x) => x.memo && String(x.memo).trim());
    if (memoFilter === "without") m = m.filter((x) => !x.memo || !String(x.memo).trim());
    const mul = sort === "dateDesc" ? -1 : 1;
    m.sort((a, b) => mul * (new Date(a.time).getTime() - new Date(b.time).getTime()));
    return m.slice(0, MAX_ROWS);
  }, [matches, filter, memoFilter, sort]);
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.55)", zIndex: Z_MATCH_LOG_MODAL, display: "flex", alignItems: isPC ? "center" : "flex-end", justifyContent: "center", animation: "fadeUp .15s ease" }}>
      <div role="dialog" aria-modal="true" aria-labelledby="match-log-modal-title" onClick={(e) => e.stopPropagation()} style={{
        ...
        <div style={{ padding: "12px 18px", borderBottom: `1px solid ${T.inp}`, display: "flex", flexDirection: "column", gap: 8 }}>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
            <span style={{ fontSize: 11, fontWeight: 600, color: T.dim, minWidth: 40 }}>{t("analysis.logFilter")}</span>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
            <span style={{ fontSize: 11, fontWeight: 600, color: T.dim, minWidth: 40 }}>{t("analysis.logMemoLabel")}</span>
            {[["any", t("analysis.logMemoAny")], ["with", t("analysis.logMemoWith")], ["without", t("analysis.logMemoWithout")]].map(([k, lab]) => (
              <button key={k} type="button" onClick={() => setMemoFilter(k)} style={{ ...sel, fontWeight: memoFilter === k ? 700 : 500, background: memoFilter === k ? T.accentSoft : T.inp, borderColor: memoFilter === k ? T.accentBorder : T.brd, color: memoFilter === k ? T.accent : T.sub }}>
                {lab}
              </button>
            ))}
