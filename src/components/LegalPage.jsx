...
本サービスには「ランク戦」と「フリー対戦」の2つのモードがあり、それぞれ対戦戦績の記録・分析機能を提供します。
...
・その他、運営者が不適切と判断する行為`,
  },
  {
    ...
    content: `利用者がメモ欄・対策メモ・振り返りメモ・フリー対戦の対戦相手名等に入力・投稿したコンテンツ（以下「利用者コンテンツ」）については、その内容・正確性・適法性について運営者は一切の責任を負いません。
...
【利用データ】
・ランク戦の対戦記録（使用キャラクター、対戦相手キャラクター、勝敗、戦闘力等）
・フリー対戦の対戦記録（対戦相手名、使用キャラクター、相手キャラクター、勝敗等）
・対策メモ・振り返りメモ等のユーザー入力テキスト
・アプリ設定情報（テーマカラー、目標設定、1日の区切り時刻等）
・サービス利用日時
...
【ローカルストレージ】
・保存される情報: ランク戦・フリー対戦の対戦データ、対策メモ、設定情報（テーマカラー、ダークモード、目標設定、1日の区切り時刻等）、ログイン状態
